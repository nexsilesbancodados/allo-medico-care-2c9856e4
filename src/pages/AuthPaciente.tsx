...
import { Phone, Mail, Lock, ArrowLeft, Heart, Check, Shield, Eye, EyeOff, Sparkles, Users, Loader2 } from "lucide-react";
...
if (Date.now() < lockoutUntil.current) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-muted/40 via-background to-background">
      ...
      {/* Header band */}
      <div className="bg-gradient-to-r from-primary to-primary/90 text-center py-8 px-6">
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5 }}
        >
          <h1 className="text-2xl font-extrabold text-primary-foreground tracking-tight italic">
            AloClínica
          </h1>
      ...
      <div className="flex-1 flex items-start justify-center px-4 -mt-4">
        <AnimatePresence mode="wait">
          ...
          {mode === "login" && (
            ...
            >
              <div className="bg-card rounded-3xl shadow-xl border border-border/40 p-7 sm:p-9">
                ...
                <form onSubmit={handleLogin} className="space-y-5">
                  <div>
                    ...
                    <div className="relative mt-1.5">
                      ...
                      <Input
                        ...
                        className="pl-11 h-12 rounded-xl bg-muted/40 border-border/60"
                        required
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative mt-1.5">
                      ...
                      <Input
                        ...
                        placeholder="••••••••"
                        className="pl-11 pr-11 h-12 rounded-xl bg-muted/40 border-border/60"
                        required
                      />
                      <button
                        type="button"
                  ...
                  <Button
                    type="submit"
                    className="w-full h-13 rounded-xl bg-primary text-primary-foreground font-bold text-base shadow-lg shadow-primary/20 hover:brightness-110 active:scale-[0.98] transition-all"
                    size="lg"
                    disabled={loading}
                  >
                    {loading ? (
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Button variant="outline" className="h-12 rounded-xl gap-2 text-sm font-medium" disabled>
                ...
                <form onSubmit={handleSignup} className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <Input value={firstName} onChange={e => setFirstName(e.target.value)} placeholder="Nome" required className="mt-1 h-11 rounded-xl" />
                    </div>
                    <div>
                      <Label className="text-xs font-semibold">Sobrenome</Label>
                      <Input value={lastName} onChange={e => setLastName(e.target.value)} placeholder="Sobrenome" required className="mt-1 h-11 rounded-xl" />
                    </div>
                  </div>
                  <div>
                    <Label className="text-xs font-semibold">CPF</Label>
                    <Input value={cpf} onChange={e => setCpf(formatMask(e.target.value, 'cpf'))} placeholder="000.000.000-00" className="mt-1 font-mono h-11 rounded-xl" maxLength={14} />
                  </div>
                  <div>
                    <Label className="text-xs font-semibold">Telefone</Label>
                    <div className="relative mt-1">
                      <Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                      <Input value={phone} onChange={e => setPhone(formatMask(e.target.value, 'phone'))} placeholder="(00) 00000-0000" className="pl-10 font-mono h-11 rounded-xl" maxLength={15} />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative mt-1">
                      ...
                      <Input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="seu@email.com" className="pl-10 h-11 rounded-xl" required />
                    </div>
                  </div>
                  <div>
