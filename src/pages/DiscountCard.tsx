...
import {
  ...
  Stethoscope, Video, Pill, Phone, Clock, ShoppingBag, Percent, Gift, Cross,
  ChevronRight, HelpCircle, Umbrella, HeartHandshake, Building2, Flower2
} from "lucide-react";
...
const plans = [
  { id: "individual", name: "Individual", price: 39.9, people: "1 pessoa", icon: <CreditCard className="w-6 h-6 text-white" />, highlighted: false, gradient: "from-primary to-primary/70" },
  { id: "couple", name: "Casal", price: 59.9, people: "2 pessoas", icon: <Heart className="w-6 h-6 text-white" />, highlighted: true, gradient: "from-primary via-primary to-secondary" },
  { id: "family", name: "Família", price: 79.9, people: "Até 5 pessoas", icon: <Users className="w-6 h-6 text-white" />, highlighted: false, gradient: "from-secondary to-secondary/70" },
];
...
const pilares = [
  ...
  },
  {
    ...
    items: ["30% off em farmácias parceiras", "Descontos em exames laboratoriais", "Parcerias com óticas e academias", "Ofertas exclusivas todo mês"],
  },
  {
...
const extras = [
  ...
  { icon: Percent, label: "Clube de descontos" },
  { icon: Gift, label: "Sem carência" },
  { icon: HeartHandshake, label: "Cobertura familiar" },
  { icon: Building2, label: "Rede credenciada" },
];

const faqs = [
  { q: "O Cartão de Vantagens é um plano de saúde?", a: "Não. O Cartão de Vantagens é um programa de benefícios que oferece acesso a telemedicina, clube de descontos e assistência funerária. Não substitui um plano de saúde." },
  ...
  { q: "A assistência funerária cobre toda a família?", a: "Sim. No plano Família, todos os membros cadastrados (até 5 pessoas) possuem cobertura completa." },
  ...
  { q: "Quais farmácias aceitam o desconto?", a: "Temos parcerias com as principais redes do Brasil. Consulte a lista completa no app após ativar seu cartão." },
];
...
const DiscountCard = () => {
  ...
  const handleSubscribe = async (planId: string, price: number) => {
    if (!user) {
      toast.info("Faça login para assinar o Cartão de Vantagens");
      navigate("/paciente");
      return;
    }
    ...
    try {
      ...
      const { data: paymentData, error: paymentError } = await supabase.functions.invoke("create-asaas-payment", {
        body: { customer_email: user.email, customer_name: user.user_metadata?.first_name || "Paciente", amount: price, description: `Cartão de Vantagens - ${planId === "individual" ? "Individual" : planId === "couple" ? "Casal" : "Família"}`, billing_type: "PIX" },
      });
      ...
      toast.success("Cartão de Vantagens ativado! 🎉", { description: "Aproveite todos os benefícios agora mesmo." });
  ...
  return (
    <>
      <SEOHead title="Cartão de Vantagens | AloClinica — Telemedicina, Descontos e Assistência" description="Telemedicina 24h, clube de descontos em farmácias e assistência funerária completa. Planos a partir de R$ 39,90/mês." />
          ...
          <div className="container mx-auto px-4 text-center relative">
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              <Badge className="mb-5 text-sm px-5 py-1.5 bg-white/15 text-white border-white/20 backdrop-blur-sm">
                <Sparkles className="w-3.5 h-3.5 mr-1.5" /> Cartão de Vantagens
              </Badge>
              ...
              <p className="text-lg text-white/70 max-w-2xl mx-auto mb-8 leading-relaxed">
                Telemedicina 24h, clube de descontos em farmácias e exames, e assistência funerária completa. Tudo em um único cartão.
              </p>
          ...
          <div className="container mx-auto px-4 max-w-5xl">
            <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true }}>
              <motion.div variants={fadeUp} className="text-center mb-14">
                ...
                <p className="text-muted-foreground mt-2 max-w-lg mx-auto">Todos os benefícios inclusos em qualquer plano, sem cobrança extra.</p>
              </motion.div>
          ...
          <div className="container mx-auto px-4">
            <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true }}>
              <motion.div variants={fadeUp} className="text-center mb-12">
                ...
                <p className="text-muted-foreground mt-2">Todos incluem Telemedicina + Clube de Descontos + Assistência Funerária</p>
              </motion.div>
              <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
                {plans.map((plan, i) => (
                  <motion.div key={plan.id} variants={fadeUp}>
                    <Card className={`relative overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl ${plan.highlighted ? "border-primary shadow-xl shadow-primary/15 ring-2 ring-primary/20" : "border-border/50 hover:shadow-xl hover:border-border"}`}>
                      {plan.highlighted && (
                        <div className="absolute top-0 left-0 right-0 bg-gradient-to-r from-primary to-secondary text-primary-foreground text-center text-xs py-1.5 font-bold tracking-wide">⭐ MAIS POPULAR</div>
                      )}
                      <CardContent className={`p-7 text-center ${plan.highlighted ? "pt-12" : ""}`}>
                        <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${plan.gradient} flex items-center justify-center mx-auto mb-5 shadow-lg`}>{plan.icon}</div>
                        ...
                        <p className="text-xs text-muted-foreground mb-6">Telemedicina + Descontos + Funerária</p>
                        <Button
                          className={`w-full h-12 rounded-xl font-bold text-base shadow-lg transition-shadow ${plan.highlighted ? "bg-gradient-to-r from-primary via-primary to-secondary text-primary-foreground shadow-primary/20 hover:shadow-xl hover:shadow-primary/30" : ""}`}
          ...
          <div className="container mx-auto px-4 max-w-4xl">
            <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true }}>
              ...
              <div className="grid sm:grid-cols-3 gap-6">
                {[
                  { step: "01", title: "Escolha seu plano", desc: "Individual, Casal ou Família. Todos com os mesmos benefícios.", icon: <CreditCard className="w-6 h-6 text-white" />, gradient: "from-primary to-primary/70" },
                  ...
                  { step: "03", title: "Use seus benefícios", desc: "Consulte, compre com desconto e tenha cobertura funeral.", icon: <HeartHandshake className="w-6 h-6 text-white" />, gradient: "from-warning to-warning/70" },
                ].map((item, i) => (
                  <motion.div key={i} variants={fadeUp}>
        ...
        <section className="py-16">
          <div className="container mx-auto px-4 max-w-3xl">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-gradient-to-br from-primary via-primary/90 to-secondary rounded-3xl p-10 text-center text-white shadow-2xl relative overflow-hidden">
              ...
              <p className="text-white/70 mb-6 max-w-md mx-auto">Ative seu Cartão de Vantagens e tenha acesso imediato a telemedicina, descontos e assistência funerária.</p>
              <Button size="lg" className="bg-white text-primary hover:bg-white/90 rounded-2xl h-14 px-10 text-base font-bold shadow-2xl" onClick={() => document.getElementById("planos")?.scrollIntoView({ behavior: "smooth" })}>
                Ver Planos <ArrowRight className="w-5 h-5 ml-2" />
              </Button>
            </motion.div>
