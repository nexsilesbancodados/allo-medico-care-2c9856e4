...
const DiscountCard = () => {
  ...
  const handleSubscribe = async (planId: string, price: number) => {
    ...
    try {
      ...
      if (existing) { toast.info("Você já possui um cartão de desconto ativo!"); setSubscribing(null); return; }
      const { data: paymentData, error: paymentError } = await supabase.functions.invoke("create-asaas-payment", {
        body: { customer_email: user.email, customer_name: user.user_metadata?.first_name || "Paciente", amount: price, description: `Cartão de Desconto - ${planId === "individual" ? "Individual" : planId === "couple" ? "Casal" : "Família"}`, billing_type: "PIX" },
      });
      ...
      toast.success("Cartão de desconto ativado com sucesso! 🎉", { description: "Você já pode aproveitar 30% de desconto em todos os serviços." });
              ...
              {plans.map((plan, i) => (
                <motion.div key={plan.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
                  <Card className={`relative overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl ${plan.highlighted ? "border-primary shadow-xl shadow-primary/15 ring-2 ring-primary/20" : "border-border/50 hover:shadow-xl hover:border-border"}`}>
                    {plan.highlighted && (
                      <div className="absolute top-0 left-0 right-0 bg-gradient-to-r from-primary to-secondary text-primary-foreground text-center text-xs py-1.5 font-bold">⭐ Mais Popular</div>
                    )}
        ...
        <section className="py-20 bg-muted/30 relative">
          <div className="container mx-auto px-4 text-center">
            <div className="relative inline-block mb-10">
              <h2 className="text-3xl font-black text-foreground tracking-tight">O que está incluso</h2>
              <p className="text-muted-foreground mt-1">Todos os planos incluem os mesmos benefícios</p>
              <motion.img
                src={pingoThumbsup}
                alt="Pingo mascote"
                className="absolute -right-16 -top-4 w-14 h-14 object-contain drop-shadow-lg hidden sm:block"
