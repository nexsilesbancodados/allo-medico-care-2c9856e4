...
import { DollarSign, AlertTriangle, Users, TrendingUp, CreditCard, FileText } from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const [stats, setStats] = useState({
    total_revenue: 0, active_subs: 0, overdue_subs: 0, total_patients: 0, total_doctors: 0, monthly_appts: 0,
  });
  ...
  const fetchAll = async () => {
    ...
    const [patientsRes, doctorsRes, activeSubsRes, expiredSubsRes, monthApptsRes, pendingRes, allSubsRes] = await Promise.all([
    ...
    // Calculate revenue from active subs
    ...
    // Enrich subs with user names and plan names
  ...
  return (
    <DashboardLayout title="Administração" nav={getAdminNav("overview")}>
      <div className="max-w-6xl">
        <h1 className="text-2xl font-bold text-foreground mb-1">Dashboard Financeiro</h1>
        <p className="text-muted-foreground mb-6">Faturamento, inadimplência e cadastros</p>
