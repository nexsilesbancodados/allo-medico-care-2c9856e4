...
import { Calendar, DollarSign, Users, TrendingUp, Video, BarChart2, ArrowRight, FileText, ExternalLink } from "lucide-react";
...
const DoctorDashboard = () => {
  ...
  interface DoctorAppt { id: string; scheduled_at: string; status: string; patient_id: string; patient_name: string; duration_minutes: number | null; }
  ...
  const queueItems: QueueItem[] = todayAppts.slice(0, 5).map(a => ({
    ...
    name: a.patient_name,
    subtitle: a.status === "in_progress" ? `Em consulta · ${format(new Date(a.scheduled_at), "HH:mm")}`
            : a.status === "waiting" ? `Aguardando · ${format(new Date(a.scheduled_at), "HH:mm")}`
    ...
    action: (a.status === "waiting" || a.status === "in_progress") ? (
      <Button size="sm" className="h-8 rounded-xl px-3 text-[10.5px] font-bold text-white bg-emerald-600 hover:bg-emerald-700"
    ...
    <DashboardLayout title="Médico" nav={getDoctorNav("home")} role="doctor">
      ...
      <div className="mx-auto w-full max-w-2xl space-y-4 pb-24">
        ...
        <HeroBanner
          ...
          liveColor="red"
          bubble={{
            greeting: `${getGreeting()}, doutor — ${format(new Date(), "dd/MM")}`,
            name: `Dr(a). ${profile?.first_name || "Médico"}`,
            sub: waitingCount > 0 ? `${waitingCount} paciente${waitingCount > 1 ? "s" : ""} na fila` : "Agenda atualizada",
          }}
          kpis={[
        ...
        <ActionPills title="Ações do médico" actions={[
          { label: "Sala de espera", icon: "🎥", iconBg: "bg-emerald-50 dark:bg-emerald-950/30", path: "/dashboard/doctor/waiting-room", badge: waitingCount },
        ...
        {queueItems.length > 0 ? (
          ...
          <div className="flex flex-col items-center py-8 text-center">
            <motion.img src={mascotWelcome} alt="Pingo" className="mb-3 h-24 w-24 object-contain select-none"
              style={{ filter: "drop-shadow(0 6px 14px rgba(0,0,0,.15))" }}
              animate={{ y: [0, -8, 0] }} transition={{ duration: 3.2, repeat: Infinity, ease: "easeInOut" }} />
            <p className="text-[13px] font-bold text-foreground">Agenda livre hoje</p>
            <p className="mt-1 text-[11px] text-muted-foreground">Nenhuma consulta agendada</p>
            <Button size="sm" className="mt-3 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700" onClick={() => navigate("/dashboard/availability")}>Configurar horários</Button>
          </div>
        )}

        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="h-10 rounded-xl border border-border/30 bg-muted/40 p-1">
          ...
          <TabsContent value="overview" className="mt-4 space-y-4">
            ...
            <PingoBannerCard
              ...
              ctaBg="bg-[#2563EB] hover:bg-[#1D4ED8]"
            ...
            {upcomingAppts.length > 0 && (
              <div className="overflow-hidden rounded-2xl border border-border/20 bg-card shadow-[0_2px_10px_rgba(0,0,0,.05)]">
                ...
                {upcomingAppts.slice(0, 4).map(a => (
                  <div key={a.id} className="flex items-center gap-3 border-b border-border/10 px-4 py-3 last:border-0 hover:bg-muted/20 transition-colors">
