      ...
      <div className="max-w-5xl space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" variant="outline" className="h-8" onClick={() => queryClient.refetchQueries({ queryKey: ["doctor-dashboard-stats"] })} disabled={refreshing}>
          ...
          <TabsContent value="overview" className="mt-5 space-y-5">
              ...
              <Card className="border-border/60">
                <CardContent className="p-4">
                  ...
                  <div className="flex flex-wrap gap-4 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1"><CheckCircle2 className="w-3 h-3 text-success" /> {done} concluída{done !== 1 ? "s" : ""}</span>
                    {inProg > 0 && <span className="flex items-center gap-1"><Video className="w-3 h-3 text-primary" /> {inProg} em andamento</span>}
                    {waitingCount > 0 && <span className="flex items-center gap-1"><Clock className="w-3 h-3 text-warning" /> {waitingCount} aguardando</span>}
                  </div>
                </CardContent>
              </Card>
            )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                <Card key={item.label} className="border-border/60 hover:shadow-md transition-all duration-200 cursor-pointer hover:-translate-y-0.5" onClick={() => navigate(item.path)}>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs sm:text-sm text-muted-foreground mt-0.5 truncate">Base de medicamentos completa do Brasil</p>
                    </div>
                  </div>
                  <div className="flex gap-2 shrink-0">
                    <Button variant="outline" size="sm" className="gap-1.5 text-xs flex-1 sm:flex-none" onClick={() => window.open("https://memed.com.br/login", "_blank")}>
                      <ExternalLink className="w-3.5 h-3.5" /> Portal
                    </Button>
                    <Button size="sm" className="bg-gradient-hero text-primary-foreground gap-1.5 text-xs flex-1 sm:flex-none" onClick={() => navigate("/dashboard/prescriptions")}>
