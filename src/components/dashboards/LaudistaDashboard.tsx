...
const LaudistaDashboard = () => {
  ...
  const dynamicSub = loadingStats
    ? "Carregando..."
    : urgentCount > 0
      ? `${urgentCount} exame${urgentCount > 1 ? "s" : ""} urgente${urgentCount > 1 ? "s" : ""}`
      : `${stats?.pending ?? 0} pendente${(stats?.pending ?? 0) !== 1 ? "s" : ""} na fila`;
            ...
            <div className="space-y-5">
              ...
              {!loadingExams && urgentCount > 0 && (
                <AlertBox
                  ...
                  onAction={() => navigate("/dashboard/laudista/queue?role=laudista")}
                />
              )}
            ...
            <div className="space-y-5">
                ...
                <Card className="border-border/50">
                  <CardHeader className="pb-3 pt-5">
                    <div className="flex items-center justify-between">
                      ...
                      <Button size="sm" variant="ghost" className="text-xs text-primary h-8 gap-1 font-semibold" onClick={() => navigate("/dashboard/laudista/queue?role=laudista")}>
                    ...
                    {loadingExams ? (
                      ...
                      <div className="space-y-2">
                        {recentExams.map(exam => (
                          <div
                            key={exam.id}
                            className={`flex items-center justify-between p-3.5 rounded-xl border transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 ${
              ...
              <motion.div variants={fadeUp}>
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { label: "Fila Completa", icon: ClipboardList, href: "/dashboard/laudista/queue?role=laudista", color: "from-primary to-secondary" },
                    { label: "Meus Laudos", icon: FileText, href: "/dashboard/laudista/my-reports?role=laudista", color: "from-secondary to-primary" },
                    { label: "Financeiro", icon: BarChart2, href: "/dashboard/laudista/financeiro?role=laudista", color: "from-success to-success/70" },
                  ].map(action => (
                    <button
                      key={action.label}
