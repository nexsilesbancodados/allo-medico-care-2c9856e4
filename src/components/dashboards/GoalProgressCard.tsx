  ...
  return (
    <div className="rounded-2xl border border-border/25 bg-card p-4" style={{ boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
      ...
      <div className="mt-2.5 flex gap-4 text-[10px] text-muted-foreground">
        <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-semibold"><CheckCircle2 className="h-3 w-3" />{done} concluídas</span>
        {inProgress > 0 && <span className="flex items-center gap-1"><Video className="h-3 w-3" />{inProgress} andamento</span>}
        {waiting > 0 && <span className="flex items-center gap-1 text-amber-600 dark:text-amber-400"><Clock className="h-3 w-3" />{waiting} aguardando</span>}
