...
import {
  usePatientStats, usePatientUpcoming, useReturnAppointments,
  useFavoriteDoctors, useRecentHealthMetrics,
...
const PatientDashboard = () => {
  ...
  const healthTips = [
    ...
    "15 min de sol ajudam na produção de vitamina D",
  ...
  const METRIC_CONFIGS: Record<string, { emoji: string; label: string; color: string }> = {
    ...
    frequencia_cardiaca: { emoji: "💓", label: "Freq. Card.",color: "text-red-500" },
    ...
    <DashboardLayout title="Paciente" nav={getPatientNav("home")} role="patient">
      ...
      <div className="mx-auto w-full max-w-2xl space-y-4 pb-24">
        ...
        <HeroBanner
          ...
          bubble={{
            greeting: `${getGreeting()} · ${format(now, "EEEE, dd 'de' MMMM", { locale: ptBR })}`,
        ...
        <StatBento loading={loading} stats={[
          { label: "Total de consultas", value: stats?.total ?? 0,         icon: "📅", iconBg: "bg-blue-50 dark:bg-blue-950/30",   valueClass: "text-[#1e3a8a] dark:text-blue-400",    accentClass: "bg-blue-500",   trend: 12 },
          { label: "Receitas emitidas",  value: stats?.prescriptions ?? 0, icon: "💊", iconBg: "bg-amber-50 dark:bg-amber-950/30", valueClass: "text-amber-600 dark:text-amber-400",    accentClass: "bg-amber-500",  trend: 5 },
        ...
        {!loading && nextAppt ? (
          <section>
            <div className="mb-2 flex items-center justify-between">
              ...
              <Button variant="link" size="sm" className="h-auto p-0 text-[11px] font-semibold text-primary" onClick={() => navigate("/dashboard/appointments?role=patient")}>
              ...
              <Card className="cursor-pointer overflow-hidden rounded-2xl border-border/20 shadow-[0_3px_14px_rgba(0,0,0,.07)]"
                ...
                <CardContent className="p-0">
                  <div className="flex items-stretch">
                    <div className={`flex w-[58px] shrink-0 flex-col items-center justify-center py-4 ${daysUntilNext === 0 ? "bg-gradient-to-b from-[#1e3a8a] to-[#2563EB] text-white" : "bg-muted/40 text-muted-foreground"}`}>
                    ...
                    <div className="flex flex-1 items-center gap-3 px-4 py-3">
                      ...
                      {daysUntilNext === 0 ? (
                        <Button size="sm" className="h-9 shrink-0 rounded-xl bg-gradient-to-br from-[#1e3a8a] to-[#2563EB] px-4 text-[11px] font-bold text-white shadow-[0_4px_14px_rgba(37,99,235,.3)]"
          ...
          <div className="flex flex-col items-center py-8 text-center">
            <motion.img
              src={mascotWelcome} alt="Pingo convidando"
              className="mb-3 h-28 w-28 object-contain select-none"
              style={{ filter: "drop-shadow(0 8px 18px rgba(0,0,0,.15))" }}
              animate={{ y: [0, -8, 0] }}
              transition={{ duration: 3.2, repeat: Infinity, ease: "easeInOut" }}
            />
            <p className="text-[14px] font-bold text-foreground">Nenhuma consulta agendada</p>
            <p className="mt-1 text-[12px] text-muted-foreground">Agende agora e cuide da sua saúde</p>
        ...
        {(healthMetrics as { type: string; value: number; unit: string }[]).length > 0 && (
          <section>
            <div className="mb-2 flex items-center justify-between">
              ...
              <Button variant="link" size="sm" className="h-auto p-0 text-[11px] font-semibold text-primary" onClick={() => navigate("/dashboard/patient/health?role=patient")}>
                Ver tudo →
              </Button>
            </div>
              ...
              {(healthMetrics as { type: string; value: number; unit: string }[]).slice(0, 6).map((m, i) => {
                ...
                return (
                  <motion.button key={m.type}
                    ...
                    className="flex flex-col items-center rounded-2xl border border-border/20 bg-card py-3 text-center shadow-[0_1px_6px_rgba(0,0,0,.06)] transition-all hover:-translate-y-1 hover:shadow-md"
                  >
                    <span className="text-[20px]">{cfg.emoji}</span>
                    <p className={`mt-1.5 text-[14px] font-black leading-none tracking-tight ${cfg.color}`}>
                      {m.value}<span className="ml-0.5 text-[8.5px] font-normal text-muted-foreground/55">{m.unit}</span>
                    </p>
                ...
                <div key={ra.id} className="mb-2 flex items-center justify-between rounded-xl border border-border/20 bg-card p-3 last:mb-0">
                  <div className="text-[11px]">
                    ...
                    <p className="mt-0.5 text-muted-foreground">
                      {daysR <= 3 ? <span className="font-semibold text-red-500">⚠️ {daysR}d restantes</span> : `Até ${format(new Date(ra.return_deadline), "dd/MM")} (${daysR}d)`}
                    </p>
                  </div>
