...
export function ActionPills({ actions, title }: ActionPillsProps) {
  ...
  return (
    <div>
      {title && <p className="mb-2.5 px-0.5 text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground/60">{title}</p>}
      <div className="flex gap-2 overflow-x-auto pb-0.5 scrollbar-none">
        {actions.map((a, i) => (
          <motion.button
            ...
            className="relative flex shrink-0 items-center gap-2 rounded-[22px] border border-border/30 bg-card px-3 py-2 shadow-[0_1px_5px_rgba(0,0,0,.07)] transition-all duration-200 hover:shadow-md active:scale-95"
          >
            <div className={cn("flex h-6 w-6 items-center justify-center rounded-[8px] text-sm", a.iconBg)}>
              {a.icon}
            </div>
