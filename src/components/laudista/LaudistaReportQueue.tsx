...
const LaudistaReportQueue = () => {
  ...
  const { data: examRequests, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Fetch patient names for exams that have patient_id
  ...
  return (
    <DashboardLayout nav={getLaudistaNav("queue")} title="Fila de Laudos" role="doctor">
      <Card>
              ...
              <Table>
                ...
                <TableBody>
                  {examRequests.map((exam: any) => (
                    <TableRow key={exam.id} className={exam.priority === "urgent" ? "bg-destructive/5" : ""}>
                      ...
                      <TableCell className="text-right space-x-2">
                        {exam.status === "pending" && (
                          <Button size="sm" variant="outline" onClick={() => handleClaim(exam.id)} disabled={claimingId === exam.id}>
